...
import { cashCouponData, AddpriTypeData, addCashData, serialCouponData, textDetailData, commitData } from '../../data/PROMOTION/Promo_AddCoupon.data';
import { Add_coupon } from '../../data/PROMOTION/Promotions_Urls';
import { addCouponPromoList, AddpriTypeDataList, secondUrl, addCashDataList } from "../../schema/PROMOTION/Promo_Addcoupon_14500.schema";
...
describe('·Navigator (Promotions Manager =>  Coupon Promotion => Create Coupon Promotion - Cash Prize)', () => {
    beforeEach(() => {
    ...
    it(a.getname(186, envSettings.envname, true), async () => {
        const NewPrm: any = await create_Coupon(envSettings.serverUrl, Add_coupon.create_Coupon(), commonheaders, cashCouponData).then((res) => {
        ...
        const Addprize: any = await create_CouponPrizeType(envSettings.serverUrl, Add_coupon.create_CouponPrizeType(promoid, prizetype), commonheaders, addCashData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        assert.jsonSchema(Addprize, addCashDataList);
        if (promoid == undefined || promoid == null) {
        ...
        assert.jsonSchema(AddText, textDetailData);

...
import { addCouponData, AddpriTypeData, AddprizeData, serialCouponData, textDetailData, commitData } from '../../data/PROMOTION/Promo_AddCoupon.data';
import { Add_coupon, Add_Buyget } from '../../data/PROMOTION/Promotions_Urls';
import { addCouponPromoList, AddpriTypeDataList, AddprizeDataList, textDetailDataList, secondUrl } from "../../schema/PROMOTION/Promo_Addcoupon_14500.schema";
import { create_Coupon, create_CouponCriteria, create_CouponPrizeType, create_CouponText, delete_CouponPromo, update_CouponCommit, read_AllPromotions } from '../../../lib/PROMOTION/Coupon';
import { buygetData } from '../../data/PROMOTION/Promo_buyGet.data';
...
describe('·Navigator (Promotions Manager =>  Coupon Promotion => Add a Coupon Promotion of Prize Type Free Prize)', () => {
    beforeEach(() => {
    ...
    it(a.getname(190, envSettings.envname, true), async () => {
        ...
        if (getAllPromoData !== null) {
            while (_.findWhere(getAllPromoData, { promotionNumber: addCouponData.promotionNumber })) {
                addCouponData.promotionNumber = RandomSource.getRandomNumber(4, true);
                addCouponData.promotionName = "test" + " " + addCouponData.promotionNumber;
            }
        } else {
            console.log('================');
        }
        const NewPrm: any = await create_Coupon(envSettings.serverUrl, Add_coupon.create_Coupon(), commonheaders, addCouponData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        ...
        assert.jsonSchema(NewPrmpromtype, secondUrl);
        if (promoid == undefined || promoid == null) {
            pending("Not able to add criteria Information");
        }
        ...
        const Addprize: any = await create_CouponPrizeType(envSettings.serverUrl, Add_coupon.create_CouponPrizeType(promoid, prizetype), commonheaders, AddprizeData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        assert.jsonSchema(Addprize, AddprizeDataList);
        if (promoid == undefined || promoid == null) {

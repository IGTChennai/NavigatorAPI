...
import { disabledCouponData, disPrizeData, AddpriTypeData, serialCouponData, textDetailData, commitData } from '../../data/PROMOTION/Promo_AddCoupon.data';
...
describe('·Navigator (Promotions Manager =>  Coupon Promotion => Add a Coupon Promotion - status Disabled)', () => {
    beforeEach(() => {
    ...
    it(a.getname(187, envSettings.envname, true), async () => {
        const NewPrm: any = await create_Coupon(envSettings.serverUrl, Add_coupon.create_Coupon(), commonheaders, disabledCouponData).then((res) => {
        ...
        const Addprize: any = await create_CouponPrizeType(envSettings.serverUrl, Add_coupon.create_CouponPrizeType(promoid, prizetype), commonheaders, disPrizeData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });

...
import { VoucherCouponData, addVoucherData, AddcriteriaDiscount, serialCouponData, textDetailData, commitData } from '../../data/PROMOTION/Promo_AddCoupon.data';
import { Add_coupon } from '../../data/PROMOTION/Promotions_Urls';
import { addCouponPromoList, discountDataListVou, AddpriTypeDataList, textDetailDataList, secondUrlVoucher } from "../../schema/PROMOTION/Promo_Addcoupon_14500.schema";
...
describe('·Navigator (Promotions Manager =>  Coupon Promotion => Add a Coupon Promotion of Prize Type Voucher Prize)', () => {
    beforeEach(() => {
    ...
    it(a.getname(192, envSettings.envname, true), async () => {
        const NewPrm: any = await create_Coupon(envSettings.serverUrl, Add_coupon.create_Coupon(), commonheaders, VoucherCouponData).then((res) => {
        ...
        assert.jsonSchema(NewPrmpromtype, secondUrlVoucher);
        if (promoid == undefined || promoid == null) {
            pending("Not able to add criteria Information");
        }
        const NewPrmCriteria: any = await create_CouponCriteria(envSettings.serverUrl, Add_coupon.create_CouponCriteria(promoid), commonheaders, AddcriteriaDiscount).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        ...
        const Addprize: any = await create_CouponPrizeType(envSettings.serverUrl, Add_coupon.create_CouponPrizeType(promoid, prizetype), commonheaders, addVoucherData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        assert.jsonSchema(Addprize, discountDataListVou);
        if (promoid == undefined || promoid == null) {

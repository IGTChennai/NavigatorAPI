...
import { discountCouponData, AddpriTypeData, textDetailDataDiscount, AddprizeDiscountData, serialCouponData, commitData } from '../../data/PROMOTION/Promo_AddCoupon.data';
import { Add_coupon } from '../../data/PROMOTION/Promotions_Urls';
import { addCouponPromoList, AddpriTypeDataList, discountDataList, textDetailDataListDiscount, secondUrl } from "../../schema/PROMOTION/Promo_Addcoupon_14500.schema";
import { create_Coupon, create_CouponCriteria, create_CouponPrizeType, create_CouponText, delete_CouponPromo, update_CouponCommit } from '../../../lib/PROMOTION/Coupon';
...
describe('·Navigator (Promotions Manager =>  Coupon Promotion => Add a Coupon Promotion of Prize Type Discount Prize)', () => {
    beforeEach(() => {
    ...
    it(a.getname(188, envSettings.envname, true), async () => {
        const NewPrm: any = await create_Coupon(envSettings.serverUrl, Add_coupon.create_Coupon(), commonheaders, discountCouponData).then((res) => {
        ...
        const Addprize: any = await create_CouponPrizeType(envSettings.serverUrl, Add_coupon.create_CouponPrizeType(promoid, prizetype), commonheaders, AddprizeDiscountData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        assert.jsonSchema(Addprize, discountDataList);
        if (promoid == undefined || promoid == null) {
            pending("Not able to add Text Information");
        }
        const AddText: any = await create_CouponText(envSettings.serverUrl, Add_coupon.create_CouponText(promoid), commonheaders, textDetailDataDiscount).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });
        assert.jsonSchema(AddText, textDetailDataListDiscount);
        if (promoid == undefined || promoid == null) {

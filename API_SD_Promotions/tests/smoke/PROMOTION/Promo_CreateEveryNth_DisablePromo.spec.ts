...
import { addNthDisableData, AddpriTypeData, addPrizeInforData, textDetailData, commitData, NthPromoData } from '../../data/PROMOTION/Promo_Nth.data';
import { Add_Nth } from '../../data/PROMOTION/Promotions_Urls';
import { addNthDataList, promoNthDat, addCriteria, addPrize } from "../../schema/PROMOTION/Promo_Nth.schema";
...
describe('·Navigator (Promotions Manager => Nth Promotion => Add a Every Nth Promo - Promo status Disabled)', () => {
    beforeEach(() => {
    ...
    it(a.getname(194, envSettings.envname, true), async () => {
        const NewPrm: any = await create_EveryNth(envSettings.serverUrl, Add_Nth.create_EveryNth(), commonheaders, addNthDisableData).then((res) => {

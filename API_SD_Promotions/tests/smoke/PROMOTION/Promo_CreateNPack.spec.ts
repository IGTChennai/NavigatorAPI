...
import { nPackData, ticketData, AddprizeData, textDetailData, commitData } from '../../data/PROMOTION/Promo_Npack.data';
import { Add_nPack, Add_Buyget } from '../../data/PROMOTION/Promotions_Urls';
import { addNpackList, prizeInfoData, TextDetailsData } from "../../schema/PROMOTION/Promo_Npack.schema";
import { create_NPack, create_NPackCriteria, create_NPackPrizeType, create_NPackText, update_NPackCommit, delete_NPackPromo, read_AllPromotions } from '../../../lib/PROMOTION/NPack';
...
describe('·Navigator (Promotions Manager => Npack Promotion => Add a NPACK -Promo status commit)', () => {
    beforeEach(() => {
    ...
    it(a.getname(199, envSettings.envname, true), async () => {
        ...
        if (getAllPromoData !== null) {
            while (_.findWhere(getAllPromoData, { promotionNumber: nPackData.promotionNumber })) {
                nPackData.promotionNumber = RandomSource.getRandomNumber(4, true);
                nPackData.promotionName = "test" + " " + nPackData.promotionNumber;
            }
        } else {
            console.log('================');
        }
        const NewPrm: any = await create_NPack(envSettings.serverUrl, Add_nPack.create_NPack(), commonheaders, nPackData).then((res) => {
            assert.equal(res.status, 200, `${res.text}`);
            return res.body;
        });

...
import { buygetdataDisable, AddpriTypeData, AddprizeData, textDetailData, commitData } from '../../data/PROMOTION/Promo_buyGet.data';
...
describe('·Navigator (Promotions Manager => Buy X, Get Y-PromoTion => Add a Every Buy X, Get Y-Promo status Disabled)', () => {
    beforeEach(() => {
    ...
    it(a.getname(183, envSettings.envname, true), async () => {
        const NewPrm: any = await create_BuyXgetY(envSettings.serverUrl, Add_Buyget.create_BuyXgetY(), commonheaders, buygetdataDisable).then((res) => {
